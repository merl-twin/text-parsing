...
impl ParserState for EntityState {
    ...
    fn next_state(self, local_char: Local<char>,  _: &Self::Context) -> NextState<EntityState,Entity> {
        match self {
            EntityState::Init => init(local_char),
            EntityState::MayBeEntity(amp_char) => may_be_entity(amp_char,local_char),
            EntityState::MayBeNumEntity(amp_char,hash_char) => may_be_num_entity(amp_char,hash_char,local_char),
            EntityState::EntityNamed(ent) => entity_named(ent,local_char),
            EntityState::EntityNumber(ent) => entity_number(ent,local_char),
            EntityState::EntityNumberX(ent) => entity_number_x(ent,local_char),
        }
    }
}

fn init(local_char: Local<char>) -> NextState<EntityState,Entity> {
...
fn entity_number_x(mut ent: ReadEntity, local_char: Local<char>) -> NextState<EntityState,Entity> {
    Ok(match *local_char.data() {
        '0' ..= '9' | 'a' ..= 'z' | 'A' ..= 'Z' => {
...
fn entity_number(mut ent: ReadEntity, local_char: Local<char>) -> NextState<EntityState,Entity> {
    Ok(match *local_char.data() {
        '0' ..= '9' => {
            ent.current = local_char;
            ent.content.push(*local_char.data());
...
fn may_be_num_entity(amp_char: Local<char>, hash_char: Local<char>, local_char: Local<char>) -> NextState<EntityState,Entity> {
    Ok(match *local_char.data() {
        'x' => {
            let ent = ReadEntity {
                begin: amp_char,
...
fn entity_named(mut ent: ReadEntity, local_char: Local<char>) -> NextState<EntityState,Entity> {
    Ok(match *local_char.data() {
...
fn may_be_entity(amp_char: Local<char>, local_char: Local<char>) -> NextState<EntityState,Entity> {
    Ok(match *local_char.data() {
